...
import { Trash2, Plus, Save, UserCircle, Settings, Award, ShieldCheck, GraduationCap, FileText, Star, Users, LogOut, ArrowLeft, UserPlus, Clock, CheckCircle, Edit3, Send, Image as ImageIcon, X, Move, Search, Folder, ChevronDown, ChevronRight, Calendar, Link as LinkIcon, Phone, Mail, Tag, Lock, MessageSquare, ChevronLeft, Sparkles, UserX, Activity, TrendingUp, Archive, Cloud, ClipboardList, CreditCard, AlertCircle, Youtube } from 'lucide-react';
...
const youtubeUtils = {
  ...
  getStandardUrl: (url) => {
    ...
    try {
      ...
      if (url.toLowerCase().includes('<iframe') && url.toLowerCase().includes('src=')) {
        const srcMatch = url.match(/src=["']([^"']+)["']/);
        if (srcMatch && srcMatch[1]) targetUrl = srcMatch[1];
      }
      const videoMatch = targetUrl.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?|shorts)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/);
      const playlistMatch = targetUrl.match(/[?&]list=([^#\&\?]+)/);
      const videoId = videoMatch ? videoMatch[1] : null;
      const playlistId = playlistMatch ? playlistMatch[1] : null;
    ...
    // Fallback to original url if valid http
          ...
          <button 
            ...
            onClick={() => {
              showModal('ADD_PACKAGE', {
                ...
                onSubmit: ({ startDate, endDate, hours, remark, hourlyRate, packageStartDate, packageEndDate, appliedCredit, manualDiscount, totalDiscount, netPrice, vatRate, vatAmount, totalPayable, extraClassesToAdd }) => {
                  let newCreditBalance = creditBalance;
                  ...
                  // Instantly Sync dynamically added Extra Classes to the student profile
                  if (extraClassesToAdd && extraClassesToAdd.length > 0) {
                     const currentExtras = student?.extraClasses || [];
                     const mergedExtras = [...currentExtras];
                     extraClassesToAdd.forEach(ext => {
                        mergedExtras.push({
                     ...
                     updatePayload.extraClasses = mergedExtras;
                  }
        ...
        <form onSubmit={handleAdd} className="p-5 border-b border-gray-200 bg-gray-50/30 flex flex-wrap gap-3 items-end">
          ...
          <div className="flex-[1.5] min-w-[150px]">
            ...
            <select value={status} onChange={e=>setStatus(e.target.value)} className="w-full border border-gray-300 rounded outline-none p-2 text-sm focus:border-indigo-500 bg-white">
              <option value="present">✔️ Present (Deduct Hrs)</option>
              <option value="makeup">🔄 Make-up Class (Deduct Hrs)</option>
              <option value="student_absent">❌ Student Absent (Keep Hrs, No Discount)</option>
              <option value="teacher_absent">⚠️ Teacher Absent (Owe Discount/Makeup)</option>
            </select>
          </div>
      ...
      <div className="bg-gray-50/80 border-b border-gray-200 p-2.5 px-4 flex justify-between items-center text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={filter} onChange={e => setFilter(e.target.value)} className="border border-gray-300 rounded py-1 px-2 text-xs focus:ring-1 focus:ring-indigo-50 outline-none bg-white font-semibold text-gray-700 cursor-pointer shadow-sm">
            ...
            <option value="student_absent">❌ Absent</option>
            <option value="teacher_absent">⚠️ Teacher Absent</option>
            <option value="added_package">⭐ Packages</option>
              ...
              {filteredAttendance.map(record => (
                <tr key={record.id} className={`hover:bg-gray-50 ${record.status === 'deleted' ? 'opacity-70 bg-gray-50/50' : ''}`}>
                  ...
                  <td className="p-3">
                    ...
                    {record.status === 'teacher_absent' && <span className="inline-flex items-center gap-1 text-amber-700 bg-amber-50 px-2 py-0.5 rounded text-xs font-bold">⚠️ Tchr. Absent {record.compensationType === 'makeup' ? '(Make-up)' : '(Discount)'}</span>}
         ...
         <div className="space-y-4">
           ...
           <div>
              ...
              <select value={formData.status} onChange={e=>setFormData({...formData, status: e.target.value})} className="w-full border border-gray-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg p-2 outline-none text-sm font-medium bg-white">
                ...
                <option value="resigned">🔴 Stopped / Resigned</option>
              </select>
           </div>
               ...
               if (info.isPrivate && !canViewPrivateCustom) {
                 return (
                   <div key={info.id} className="flex flex-col gap-2 mb-3 bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                     <div className="text-sm text-gray-500 flex items-center gap-1.5"><Lock size={14} className="text-gray-400"/> {info.label || 'Private Field'} — Hidden</div>
                   </div>
                 );
               }
...
function StudentVideoCard({ student, onSave, canEdit }) {
  const [isEditing, setIsEditing] = useState(false);
  const [url, setUrl] = useState(student?.youtubeUrl || '');

  useEffect(() => {
    setUrl(student?.youtubeUrl || '');
  }, [student?.youtubeUrl]);

  const handleSave = () => {
    onSave({ youtubeUrl: url });
    setIsEditing(false);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mt-6">
      <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
        <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
          <Youtube className="text-red-500" size={20}/> Class Videos
        </h3>
        {!isEditing && canEdit && (
          <button onClick={() => setIsEditing(true)} className="text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 p-1.5 rounded-lg transition-colors" title="Edit Video Link">
            <Edit3 size={16}/>
          </button>
        )}
      </div>
      ...
      {isEditing ? (
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">YouTube Playlist or Video Link</label>
            <input type="text" className="w-full border border-gray-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg p-2 outline-none text-sm font-medium" value={url} onChange={e=>setUrl(e.target.value)} placeholder="Paste YouTube link here..." />
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={()=>setIsEditing(false)} className="flex-1 bg-gray-100 text-gray-600 hover:bg-gray-200 py-2 rounded-lg text-sm font-bold transition-colors">Cancel</button>
            <button onClick={handleSave} className="flex-1 bg-indigo-600 text-white hover:bg-indigo-700 py-2 rounded-lg text-sm font-bold transition-colors shadow-sm">Save Link</button>
          </div>
        </div>
      ) : (
        <div>
          {student.youtubeUrl ? (
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 flex flex-col items-center text-center gap-3">
              <div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center border border-gray-100">
                <Youtube size={24} className="text-red-500" />
              </div>
              <div>
                <h4 className="font-bold text-gray-800 text-sm">Class Video Linked</h4>
                <p className="text-xs text-green-600 font-bold mt-1 flex items-center justify-center gap-1"><CheckCircle size={14} /> Ready to watch</p>
              </div>
              <a href={youtubeUtils.getStandardUrl(student.youtubeUrl)} target="_blank" rel="noopener noreferrer" className="w-full bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-lg text-sm font-bold shadow-sm transition-colors mt-1">
                Open in YouTube
              </a>
            </div>
          ) : (
            <div className="text-center text-gray-500 italic text-sm p-6 bg-gray-50 rounded-lg border border-gray-100 flex flex-col items-center">
              <Youtube size={32} className="text-gray-300 mb-2" />
              <p>No video playlist assigned yet.</p>
            </div>
          )}
        </div>
      )}
          ...
          <div className={`ml-24 flex-1 flex flex-col ${isSpelling ? 'pt-6 px-10 pb-6' : 'pt-8 px-12 pb-12'} relative bg-white z-20 h-[1123px] box-border`}>
            ...
            <div className={`relative ${isSpelling ? 'mb-2' : 'mb-4'}`}>
              <div className="absolute inset-0 z-0 flex flex-col items-center justify-center opacity-[0.06] pointer-events-none select-none overflow-hidden pb-12">
                ...
                <div className="text-xl font-sans mt-2 text-center text-[#213568] font-bold z-10 opacity-70">โรงเรียนสอนภาษา อ๊อกซ์ฟอร์ด</div>
              </div>
...
function AddPackageForm({ schedule, creditBalance = 0, onSubmit, onCancel }) {
  ...
  const [excludedSessionIds, setExcludedSessionIds] = useState([]); // Store unique session IDs
  
  // Extra Classes specific to this package
  const [extraDates, setExtraDates] = useState([]);
  const [newExtraDate, setNewExtraDate] = useState('');
  const [newExtraStartTime, setNewExtraStartTime] = useState('17:00');
  const [newExtraEndTime, setNewExtraEndTime] = useState('18:30');
  ...
  const handleAddExtraDate = () => {
     if(!newExtraDate || !newExtraStartTime || !newExtraEndTime) return;
     setExtraDates(prev => [...prev, {
        id: `temp_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        dateStr: newExtraDate,
        startTime: newExtraStartTime,
        endTime: newExtraEndTime,
        hours: getDurationHours(newExtraStartTime, newExtraEndTime),
        isExtra: true
     }]);
     setNewExtraDate('');
  };

  // 1. Generate all theoretical classes based on schedule and dynamically added extra dates
  const generatedSessions = React.useMemo(() => {
    if (!packageStartDate || !packageEndDate) return [];
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        ...
        if (scheduleMap.has(dayOfWeek)) {
            ...
            sessions.push({
                id: `sched_${d.toISOString().split('T')[0]}_${sched.startTime}`,
    ...
    // Merge manually added extra dates into the preview list
    extraDates.forEach(ext => {
        sessions.push({
            id: ext.id,
            dateStr: ext.dateStr,
            dayOfWeek: new Date(ext.dateStr).getDay(),
            startTime: ext.startTime,
            endTime: ext.endTime,
            hours: ext.hours,
            isExtra: true
    ...
    return sessions;
  }, [packageStartDate, packageEndDate, schedule, extraDates]);

  // 2. Filter out sessions the admin unchecked
  const activeSessions = React.useMemo(() => {
    return generatedSessions.filter(s => !excludedSessionIds.includes(s.id));
  }, [generatedSessions, excludedSessionIds]);
  ...
  return (
    <form onSubmit={e => { 
      ...
      onSubmit({ 
        ...
        vatAmount,
        totalPayable,
        extraClassesToAdd: activeSessions.filter(s => s.isExtra).map(s => ({
           date: s.dateStr,
      ...
      {/* NEW EXTRA DATES BUILDER */}
      <div className="mb-4 bg-purple-50 rounded-xl border border-purple-200 p-4 shadow-sm">
         <label className="block text-sm font-bold text-purple-900 mb-1">Add Extra / Temporary Dates</label>
         <p className="text-[11px] text-purple-700 font-medium mb-3">Include ad-hoc dates into the package calculation.</p>
         <div className="flex flex-wrap gap-2 items-end">
            <div className="flex-1 min-w-[130px]">
               <label className="block text-[10px] font-extrabold text-purple-600 uppercase tracking-wider mb-1">Date</label>
               <input type="date" value={newExtraDate} onChange={e=>setNewExtraDate(e.target.value)} className="w-full border border-purple-200 rounded p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500 bg-white" />
            </div>
            <div className="w-24">
               <label className="block text-[10px] font-extrabold text-purple-600 uppercase tracking-wider mb-1">Start Time</label>
               <input type="time" value={newExtraStartTime} onChange={e=>setNewExtraStartTime(e.target.value)} className="w-full border border-purple-200 rounded p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500 bg-white text-center" />
            </div>
            <div className="w-24">
               <label className="block text-[10px] font-extrabold text-purple-600 uppercase tracking-wider mb-1">End Time</label>
               <input type="time" value={newExtraEndTime} onChange={e=>setNewExtraEndTime(e.target.value)} className="w-full border border-purple-200 rounded p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500 bg-white text-center" />
            </div>
            <button type="button" onClick={handleAddExtraDate} className="bg-purple-600 text-white font-bold px-4 rounded shadow-sm hover:bg-purple-700 transition-colors h-[34px] text-sm">
               Add
            </button>
         </div>
      </div>
         ...
         <div className="mb-4">
            ...
            <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg bg-gray-50 p-2 space-y-1">
               {generatedSessions.map(session => {
                  const isChecked = !excludedSessionIds.includes(session.id);
                  const dayName = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][session.dayOfWeek];
                  return (
                    <label key={session.id} className={`flex items-center gap-3 p-2 rounded cursor-pointer transition-colors ${isChecked ? 'bg-white shadow-sm border border-gray-200' : 'bg-gray-100 opacity-60'} ${session.isExtra && isChecked ? 'border-purple-200 bg-purple-50/50' : ''}`}>
                       <input 
                          type="checkbox" 
                          checked={isChecked}
                          onChange={(e) => {
                       ...
                       <div className={`flex-1 text-sm font-medium ${isChecked ? 'text-gray-800' : 'text-gray-400 line-through'}`}>
                          ...
                          {session.isExtra && <span className="ml-2 text-[10px] bg-purple-100 text-purple-700 border border-purple-200 px-1.5 py-0.5 rounded font-extrabold uppercase tracking-wider">Extra</span>}
                       </div>
                       <div className={`text-sm font-bold ${isChecked ? 'text-indigo-600' : 'text-gray-400'} ${session.isExtra && isChecked ? 'text-purple-700' : ''}`}>
...
function ReportIssueForm({ currentUser, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={e => {
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={onCancel} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-bold hover:bg-gray-200">Cancel</button>
        <button type="submit" className="px-5 py-2.5 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700 shadow-sm primary-action">Submit Report</button>
      </div>
    </form>
  );
}
...
function UpdateTicketForm({ ticket, onSubmit, onCancel }) {
  const [status, setStatus] = useState(ticket.status || 'open');
  const [adminRemark, setAdminRemark] = useState(ticket.adminRemark || '');

  return (
    <form onSubmit={e => {
      e.preventDefault();
      onSubmit({ ...ticket, status, adminRemark, updatedAt: new Date().toISOString() });
    }} className="p-6">
      <h3 className="text-xl font-bold mb-4 text-gray-900 flex items-center gap-2"><Edit3 className="text-indigo-600" /> Update Support Ticket</h3>
      <div className="space-y-4 mb-6">
        <div>
          <label className="block text-sm font-bold mb-1 text-gray-700">Ticket Status</label>
          <select value={status} onChange={e=>setStatus(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-indigo-500 bg-white font-medium text-gray-800 shadow-sm cursor-pointer">
        ...
        <div>
          <label className="block text-sm font-bold mb-1 text-gray-700">Admin Remark (Optional)</label>
          <textarea value={adminRemark} onChange={e=>setAdminRemark(e.target.value)} rows="3" className="w-full border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm" placeholder="Leave notes about the fix, delays, or follow-ups here..."></textarea>
        </div>
      </div>
      <div className="flex justify-end gap-3">
        <button type="button" onClick={onCancel} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-bold hover:bg-gray-200 transition-colors">Cancel</button>
        <button type="submit" className="px-5 py-2.5 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 shadow-sm primary-action transition-colors">Save Update</button>
      </div>
    </form>
  );
}

// COMPONENTS: Admin Ticket Dashboard
function AdminTicketDashboard({ type, tickets, showModal, title, description, icon }) {
  ...
  const monthlyStats = React.useMemo(() => {
    ...
    tickets.forEach(t => {
      ...
      if (!stats[m]) stats[m] = { total: 0, resolved: 0, in_progress: 0, open: 0, month: m };
      stats[m].total++;
      if (t.status === 'resolved') stats[m].resolved++;
      else if (t.status === 'in_progress') stats[m].in_progress++;
      else if (t.status === 'open') stats[m].open++;
    });
    return Object.values(stats).sort((a, b) => b.month.localeCompare(a.month));
  }, [tickets]);

  const currentMonthStats = summaryMonth === 'all' 
    ? {
    ...
    : (monthlyStats.find(s => s.month === summaryMonth) || { total: 0, resolved: 0, in_progress: 0, open: 0, month: summaryMonth });
  ...
  return (
    <div className="flex flex-col flex-1 bg-gray-50/50 p-6 overflow-y-auto print:p-0 print:bg-white print:overflow-visible print:block">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center print:hidden">
        <div>
          <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">{icon} {title}</h3>
          <p className="text-sm text-gray-500 mt-1">{description}</p>
        </div>
        <div className="flex gap-3 items-center">
            <select value={summaryMonth} onChange={e => setSummaryMonth(e.target.value)} className="border border-gray-300 rounded-lg py-2 px-4 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-indigo-50 shadow-sm cursor-pointer bg-white">
              <option value="all">All Time</option>
              {monthlyStats.map(m => (
                <option key={m.month} value={m.month}>{formatMonthLabel(m.month)}</option>
              ))}
              {!monthlyStats.find(m => m.month === new Date().toISOString().slice(0, 7)) && (
                <option value={new Date().toISOString().slice(0, 7)}>{formatMonthLabel(new Date().toISOString().slice(0, 7))}</option>
              )}
            </select>
            <button type="button" onClick={handlePrintReport} className="px-4 py-2 bg-white text-indigo-700 rounded-lg font-bold hover:bg-indigo-50 transition-colors border-2 border-indigo-600 shadow-sm flex items-center gap-2">
              <FileText size={16}/> Print Report
            </button>
        </div>
      </div>

      <div id="print-report" className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm print:shadow-none print:border-none print:p-0 print:m-0">
          <style dangerouslySetInnerHTML={{ __html: `
            @media print {
              body * { visibility: hidden; }
              #print-report, #print-report * { visibility: visible; }
              #print-report { position: absolute; left: 0; top: 0; width: 100%; margin: 0 !important; padding: 0 !important; border: none !important; box-shadow: none !important; border-radius: 0 !important; }
              @page { size: A4 portrait; margin: 10mm; }
              table { width: 100% !important; border-collapse: collapse !important; table-layout: fixed !important; }
              thead { display: table-header-group; }
              tr { page-break-inside: avoid; border-bottom: 1px solid #d1d5db !important; }
              th, td { word-wrap: break-word !important; font-size: 11px !important; padding: 8px !important; }
          ...
          <div className="text-center border-b-2 border-[#213568] pb-6 mb-8">
              ...
              <h2 className="text-xl font-bold text-gray-600 uppercase tracking-widest mt-2">{title}</h2>
          </div>

          <div className="grid grid-cols-2 gap-y-4 gap-x-8 mb-10 text-[15px]">
              <div className="flex items-center"><span className="font-bold text-gray-500 w-40 uppercase text-xs tracking-wider">Report Period</span> <span className="font-bold text-gray-900">{summaryMonth === 'all' ? 'All Time' : formatMonthLabel(summaryMonth)}</span></div>
              <div className="flex items-center"><span className="font-bold text-gray-500 w-40 uppercase text-xs tracking-wider">Active Items</span> <span className="font-bold text-red-600">{tickets.filter(t => t.status === 'open' || t.status === 'in_progress').length} System-wide</span></div>
              <div className="flex items-center"><span className="font-bold text-gray-500 w-40 uppercase text-xs tracking-wider">Report Generated</span> <span className="font-bold text-gray-900">{new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}</span></div>
          </div>
            ...
            <table className="w-full text-sm text-left border-collapse">
                <tbody className="divide-y divide-gray-200">
                  <tr className="bg-gray-50">
                      <td className="p-4 font-bold text-gray-700 w-1/2">Total Items Reported</td>
                      <td className="p-4 font-extrabold text-indigo-700">{currentMonthStats.total}</td>
                  </tr>
                  <tr className="bg-white">
                      <td className="p-4 font-bold text-gray-700">Resolved (Fixed)</td>
                      <td className="p-4 font-extrabold text-green-600">{currentMonthStats.resolved}</td>
                  </tr>
                  <tr className="bg-gray-50">
                      <td className="p-4 font-bold text-gray-700">In Progress</td>
                      <td className="p-4 font-extrabold text-amber-600">{currentMonthStats.in_progress}</td>
                  </tr>
                  <tr className="bg-white">
                      <td className="p-4 font-bold text-gray-700">New / Open</td>
                      <td className="p-4 font-extrabold text-red-600">{currentMonthStats.open}</td>
                  </tr>
                  <tr className="bg-indigo-50 border-t-2 border-indigo-200">
                      <td className="p-4 font-extrabold text-[#213568] uppercase tracking-wider text-xs">Resolution Rate</td>
                      <td className="p-4 font-extrabold text-indigo-700 text-lg">{currentMonthStats.total > 0 ? Math.round((currentMonthStats.resolved / currentMonthStats.total) * 100) : 0}%</td>
                  </tr>
                </tbody>
            </table>
          </div>

          <h3 className="text-lg font-bold text-gray-800 mb-4 border-b border-gray-200 pb-2">Detailed Records</h3>
          <div className="overflow-hidden rounded-xl border border-gray-300 mb-4">
            <table className="w-full text-[13px] sm:text-sm text-left border-collapse">
              <thead className="bg-gray-100 text-gray-600 border-b border-gray-300">
                <tr>
                  <th className="p-3 font-bold uppercase tracking-wider border-r border-gray-300 w-32">Date</th>
                  <th className="p-3 font-bold uppercase tracking-wider border-r border-gray-300 w-40">Reported By</th>
                  <th className="p-3 font-bold uppercase tracking-wider border-r border-gray-300">Description</th>
                  <th className="p-3 font-bold uppercase tracking-wider border-r border-gray-300 w-32 text-center">Status</th>
                  <th className="p-3 font-bold uppercase tracking-wider w-32 text-center print:hidden">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {(() => {
                  const displayedTickets = tickets
                    .filter(t => summaryMonth === 'all' || t.createdAt.startsWith(summaryMonth))
                    .sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
                  
                  if (displayedTickets.length === 0) {
                      return <tr><td colSpan="5" className="p-8 text-center text-gray-500 italic bg-gray-50">No items found for this period.</td></tr>;
                  }

                  return displayedTickets.map(ticket => (
                    <tr key={ticket.id} className={`bg-white hover:bg-gray-50 transition-colors ${ticket.status === 'resolved' ? 'opacity-70' : ''}`}>
                      <td className="p-3 border-r border-gray-200 font-medium text-gray-800 align-top">
                        {new Date(ticket.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                        <div className="text-[10px] text-gray-500 mt-1 font-bold">{new Date(ticket.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</div>
                      </td>
                      <td className="p-3 border-r border-gray-200 align-top">
                        <div className="font-bold text-indigo-700">{ticket.userName}</div>
                        <div className="text-[10px] uppercase font-extrabold text-gray-500 tracking-wider mt-1">{ticket.userRole}</div>
                      </td>
                      <td className="p-3 border-r border-gray-200 text-gray-700 align-top text-sm leading-relaxed">
                        {ticket.ticketType === 'feedback' && type === 'issue' && (
                          <div className="mb-2">
                            <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wider shadow-sm">
                              <MessageSquare size={10}/> Parent Feedback
                            </span>
                          </div>
                        )}
                        <div className="whitespace-pre-wrap">{ticket.issue}</div>
                        {ticket.adminRemark && (
                          <div className="mt-2 bg-indigo-50/40 p-2 rounded-lg border border-indigo-100 text-xs text-gray-700 shadow-sm">
                            <span className="font-bold text-indigo-800 uppercase text-[9px] tracking-wider block mb-0.5">Admin Remark</span>
                            {ticket.adminRemark}
                          </div>
                        )}
                      </td>
                      <td className="p-3 border-r border-gray-200 text-center align-top">
                        <span className={`inline-block px-2 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wider whitespace-nowrap shadow-sm border ${
                      ...
                      <td className="p-3 text-center align-top print:hidden">
                        <div className="flex flex-col gap-1.5 items-center">
                            <button onClick={() => showModal('UPDATE_TICKET', { ticket, onSubmit: (data) => updateDoc(getPublicDoc('tickets', ticket.id), data) })} className="w-full bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 py-1.5 rounded text-[11px] font-bold shadow-sm transition-colors">Update</button>
                            <button onClick={() => deleteDoc(getPublicDoc('tickets', ticket.id))} className="w-full text-gray-400 hover:text-red-600 bg-gray-50 hover:bg-red-50 border border-gray-200 hover:border-red-200 py-1.5 rounded text-[11px] font-bold transition-colors">Delete</button>
                        </div>
                      </td>
                    </tr>
                  ));
...
function StudentDashboardPage({ student, currentUser, db, showModal, allCalendarEvents, onNavigate }) {
  const { t } = useTranslation();
  ...
  const navItems = [
    ...
    { id: 'videos', label: t('class_videos') || 'Class Videos', icon: <Youtube size={18} /> },
    ...
    { id: 'feedback_history', label: 'Feedback History', icon: <MessageSquare size={18} /> },
  ];
      ...
      <div className="flex-1 flex flex-col gap-6">
           ...
           <>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 flex flex-col gap-6">
                   ...
                   {/* NEW CLASS VIDEO WIDGET ON DASHBOARD */}
                   <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col">
                      <h3 className="text-sm font-extrabold text-[#213568] uppercase tracking-widest flex items-center gap-2 mb-4">
                        <Youtube size={18} className="text-red-500" /> Recent Class Video
                      </h3>
                      {student.youtubeUrl ? (
                         <div className="flex-1 flex flex-col items-center justify-center bg-red-50 rounded-xl border border-red-100 p-5 text-center gap-3">
                            <div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center border border-red-100">
                              <Youtube size={24} className="text-red-500" />
                            </div>
                            <p className="text-sm font-bold text-red-900">Your class video playlist is ready!</p>
                            <a href={youtubeUtils.getStandardUrl(student.youtubeUrl)} target="_blank" rel="noopener noreferrer" className="w-full mt-2 bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-sm text-sm">
                               Watch on YouTube
                            </a>
                         </div>
                      ) : (
                         <div className="flex-1 flex flex-col items-center justify-center text-gray-400 italic text-sm py-6 bg-gray-50 rounded-lg border border-gray-100">
                            <Youtube size={24} className="mb-2 text-gray-300"/>
         ...
         {activeTab === 'videos' && (
           <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[500px] flex flex-col">
              <div className="p-5 border-b border-gray-100 bg-gray-50 shrink-0">
                <h2 className="text-lg font-bold text-[#213568] flex items-center gap-2"><Youtube size={20} className="text-red-500"/> {t('class_videos') || 'Class Videos'}</h2>
              </div>
              <div className="p-6 flex-1 bg-gray-50/30 flex flex-col items-center justify-center">
                {student.youtubeUrl ? (
                   <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm w-full max-w-lg text-center flex flex-col items-center gap-4">
                      <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mb-2">
                         <Youtube size={40} className="text-red-500" />
                      </div>
                      <h3 className="text-xl font-bold text-gray-900">Class Video Playlist</h3>
                      <p className="text-gray-600 mb-6 font-medium">Click the button below to open and watch the recorded class sessions securely on YouTube.</p>
                      <a href={youtubeUtils.getStandardUrl(student.youtubeUrl)} target="_blank" rel="noopener noreferrer" className="bg-red-600 hover:bg-red-700 text-white px-8 py-3.5 rounded-xl font-bold shadow-md flex items-center justify-center gap-2 transition-colors text-lg w-full sm:w-auto">
                         <Youtube size={24} /> Watch on YouTube
                      </a>
                   </div>
                ) : (
                   <div className="text-center text-gray-500 italic p-12 bg-white rounded-xl border border-gray-200 shadow-sm w-full max-w-xl">
                      <Youtube size={48} className="mx-auto text-gray-300 mb-4" />
                      <p>No videos or playlists have been uploaded for this student yet.</p>
                   </div>
                )}
              </div>
           </div>
         ...
         {activeTab === 'feedback_history' && (
           <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[500px] flex flex-col">
              <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
                <h2 className="text-lg font-bold text-[#213568] flex items-center gap-2"><MessageSquare size={20}/> Feedback History</h2>
              </div>
              <div className="p-6 flex-1 overflow-y-auto bg-gray-50/30">
                 {(() => {
                   const myTickets = (db?.tickets || [])
                      .filter(t => t.userId === currentUser.id && t.ticketType === 'feedback')
                      .sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
                   
                   if (myTickets.length === 0) {
                     return (
                       <div className="text-center text-gray-500 italic p-12 bg-white rounded-xl border border-gray-200 shadow-sm">
                   ...
                   return (
                     <div className="space-y-4">
                       {myTickets.map(ticket => (
                         <div key={ticket.id} className={`border p-5 rounded-xl shadow-sm transition-colors ${ticket.status === 'resolved' ? 'bg-gray-50 border-gray-200 opacity-80' : 'bg-white border-blue-100'}`}>
                           <div className="flex justify-between items-start mb-3 border-b border-gray-100 pb-3">
                             <div className="text-xs font-bold text-gray-500">{new Date(ticket.createdAt).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}</div>
                             <span className={`inline-block px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wider whitespace-nowrap shadow-sm border ${
                                ticket.status === 'resolved' ? 'bg-green-50 text-green-700 border-green-200' : 
                                ticket.status === 'in_progress' ? 'bg-amber-50 text-amber-700 border-amber-200' : 
                           ...
                           <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed font-medium">{ticket.issue}</p>
                           {ticket.adminRemark && (
                ...
                {filteredReports.map(report => {
                  ...
                  return (
                    <tr key={report.id} className="hover:bg-indigo-50/30 transition-colors bg-white group">
                      ...
                      <td className="p-4 font-bold text-gray-900">{student ? student.name.split(' ')[0] : 'Unknown'}</td>
...
function AttendanceSummaryReport({ student, onClose }) {
  ...
  const getScheduledTime = (dateString) => {
    ...
    return sched ? `${sched.startTime} – ${sched.endTime}` : '-';
  };
  ...
  // Increased chunk limits so more rows fit perfectly onto the first printed page
  const chunks = getChunks(reportData.finalList, 20, 30);
  ...
  return (
    <div className="w-full max-w-4xl bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 print:shadow-none print:border-none mb-12">
      <style dangerouslySetInnerHTML={{ __html: `
        #attendance-report-container table tr:last-child td { border-bottom: 1px solid #d1d5db !important; }
        @media print {
          @page { size: A4 portrait; margin: 10mm; }
          ...
          .print-page { width: 100% !important; max-width: none !important; page-break-after: always; padding: 0 !important; }
          ...
          th, td { border: 1px solid #d1d5db !important; word-wrap: break-word !important; font-size: 11px !important; padding: 6px 8px !important; }
          table tr:last-child td { border-bottom: 1px solid #d1d5db !important; }
...
function AdvancedCourseSummaryReport({ student, onClose }) {
  ...
  // Increased chunk limits so more rows fit perfectly onto the first printed page
  const chunks = getChunks(tableData, 20, 30);

  return (
    <div className="w-full max-w-4xl bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 print:shadow-none print:border-none mb-12">
      <style dangerouslySetInnerHTML={{ __html: `
        #advanced-report-container table tr:last-child td { border-bottom: 1px solid #d1d5db !important; }
        @media print {
          @page { size: A4 portrait; margin: 10mm; }
          body { background: white !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; }
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubTickets = onSnapshot(getPublicCollection('tickets'), (snapshot) => {
      setDb(prev => ({ ...prev, tickets: snapshot.docs.map(d => d.data()) }));
    }, console.error);
  ...
  const allCalendarEvents = React.useMemo(() => {
    ...
    db.students.filter(s => s.status !== 'resigned').forEach(student => {
      ...
      if (match) {
         ...
         studentDisplay = student.name.split(' ')[0];
      }
    ...
    db.students.forEach(student => {
      (student.attendance || []).forEach(record => {
        ...
        else studentDisplay = student.name.split(' ')[0];
    ...
    showModal('ADD_STUDENT', {
      onSubmit: (newStudentName) => {
        const newStudent = {
          ...
          certificateStatus: 'none', certificateDate: null, certificateCode: null, reports: [], youtubeUrl: ''
        };
  ...
  const handleDeleteStudent = (studentId) => {
    showModal('CONFIRM', {
      ...
      message: 'Are you sure you want to permanently delete this student? This action cannot be undone and will also remove their portal login access.',
      onConfirm: () => {
        // Find and delete any linked login account
        const linkedUser = db.users.find(u => u.studentId === studentId);
        if (linkedUser) {
          deleteDoc(getPublicDoc('users', linkedUser.id)).catch(console.error);
        }
        
        // Delete the main student document
        deleteDoc(getPublicDoc('students', studentId)).then(() => {
          if (currentStudentId === studentId) {
            setCurrentStudentId(null);
            setCurrentView('dashboard');
          }
          showModal('ALERT', { title: 'Deleted', message: 'Student profile successfully deleted.' });
        }).catch(console.error);
      }
    });
  };
  ...
  return (
    <LanguageContext.Provider value={{ language, setLanguage, t }}>
      ...
      {!currentUser ? (
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col font-sans print:block print:h-auto print:bg-white print:overflow-visible">
              ...
              {(currentUser.role === 'student' || currentUser.role === 'parent') ? (
                ...
                <button 
                  onClick={() => showModal('REPORT_ISSUE', { 
                    currentUser, 
                    onSubmit: (data) => setDoc(getPublicDoc('tickets', data.id), data).catch(console.error)
                  })} 
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 border border-red-100 rounded-lg text-xs font-bold transition-colors shadow-sm" 
          ...
          <div className="flex-1 p-6 flex justify-center items-start overflow-y-auto print:p-0 print:block print:overflow-visible">
            ...
            {currentUser.role === 'superadmin' && currentView === 'dashboard' && (
              <div className="w-full max-w-6xl bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 flex flex-col print:shadow-none print:border-none print:w-full print:max-w-none print:overflow-visible print:block">
                ...
                <div className="flex flex-col md:flex-row flex-1 min-h-[600px] print:block print:overflow-visible print:min-h-0">
                  {/* SIDEBAR NAVIGATION */}
                  <div className="w-full md:w-64 shrink-0 bg-gray-50 border-r border-gray-200 p-4 flex flex-col gap-2 print:hidden">
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          <button onClick={() => setAdminTab('tickets')} className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all w-full text-left ${adminTab === 'tickets' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200 hover:text-indigo-600'}`}>
                            <span className="flex items-center gap-3"><AlertCircle size={18}/> Support Tickets</span>
                            {openIssuesCount > 0 && (
                              <span className={`text-[10px] px-2 py-0.5 rounded-full ${adminTab === 'tickets' ? 'bg-white text-red-600' : 'bg-red-500 text-white'}`}>{openIssuesCount}</span>
                            )}
                          </button>
                          <button onClick={() => setAdminTab('feedback')} className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all w-full text-left ${adminTab === 'feedback' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200 hover:text-indigo-600'}`}>
                            <span className="flex items-center gap-3"><MessageSquare size={18}/> Parent Feedback</span>
                            {openFeedbackCount > 0 && (
                              <span className={`text-[10px] px-2 py-0.5 rounded-full ${adminTab === 'feedback' ? 'bg-white text-blue-600' : 'bg-blue-500 text-white'}`}>{openFeedbackCount}</span>
                            )}
                          </button>
                        </>
                      );
                  ...
                  {/* MAIN CONTENT AREA */}
                  <div className="flex-1 flex flex-col bg-white overflow-hidden relative print:block print:overflow-visible">
                    {adminTab === 'students' && (
                                              ...
                                              {group.students.map(student => (
                                                <tr key={student.id} className="hover:bg-gray-50/80 transition-colors">
                                                  ...
                                                  <td className="p-4 text-right flex justify-end gap-2 items-center">
                                                    ...
                                                    <button onClick={() => handleDeleteStudent(student.id)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-colors border border-transparent hover:border-red-200" title="Delete Student"><Trash2 size={18} /></button>
                                                  </td>
                                                </tr>
                                              ))}
                                            </tbody>
                    ...
                    {adminTab === 'feedback' && (
                      <AdminTicketDashboard 
                        ...
                        description="Review and respond to messages and feedback from parents."
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              {todaysClasses.map((cls, idx) => (
                                 <div key={idx} className="bg-white border border-indigo-100 p-5 rounded-xl shadow-sm flex flex-col hover:border-indigo-300 transition-colors">
                                    ...
                                    {/* Quick YouTube Link Input for Teacher */}
                                    <div className="mb-4 bg-gray-50 p-2.5 rounded-lg border border-gray-100" onClick={(e) => e.stopPropagation()}>
                                       <div className="flex items-center justify-between mb-1.5">
                                          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1"><Youtube size={12} className="text-red-500"/> YouTube Link</label>
                                          {cls.student.youtubeUrl && <span className="text-green-600 text-[10px] font-bold flex items-center gap-0.5"><CheckCircle size={10}/> Linked</span>}
                                       </div>
                                       {!cls.student.youtubeUrl ? (
                                         <input 
                                            type="text" 
                                            defaultValue={cls.student.youtubeUrl || ''} 
                                            onBlur={(e) => {
                                               if (e.target.value !== (cls.student.youtubeUrl || '')) {
                                                  updateDoc(getPublicDoc('students', cls.student.id), { youtubeUrl: e.target.value }).catch(console.error);
                                               }
                                            }}
                                            className="w-full border border-gray-300 rounded p-1.5 text-xs outline-none focus:ring-1 focus:ring-indigo-500 bg-white shadow-sm"
                                            placeholder="Paste YouTube link..."
                                         />
                                       ) : (
                                         <div className="flex gap-1.5">
                                            <a href={youtubeUtils.getStandardUrl(cls.student.youtubeUrl)} target="_blank" rel="noopener noreferrer" className="flex-1 bg-white border border-gray-300 text-xs font-bold text-gray-700 py-1.5 rounded flex items-center justify-center hover:bg-gray-50 shadow-sm transition-colors">
                                              Test Link
                                            </a>
                                            <button onClick={() => updateDoc(getPublicDoc('students', cls.student.id), { youtubeUrl: '' }).catch(console.error)} className="bg-white border border-gray-300 text-red-500 px-2.5 py-1.5 rounded hover:bg-red-50 shadow-sm transition-colors" title="Remove Link">
                                              <Trash2 size={14}/>
                                            </button>
                                         </div>
                                       )}
                      ...
                      {db.students.filter(s => s.teacherId === currentUser.id && s.status !== 'resigned').map(student => (
                        <div key={student.id} className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden flex flex-col cursor-pointer group" onClick={() => { setCurrentStudentId(student.id); setCurrentView('student_profile'); }}>
                          ...
                          <div className="pt-10 px-6 pb-6 flex-1 flex flex-col">
                            ...
                            {/* Quick YouTube Link Input for Teacher */}
                            <div className="mt-3 bg-gray-50 p-2.5 rounded-lg border border-gray-100" onClick={(e) => e.stopPropagation()}>
                               <div className="flex items-center justify-between mb-1.5">
                                 <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1"><Youtube size={12} className="text-red-500"/> YouTube Link</label>
                                 {student.youtubeUrl && <span className="text-green-600 text-[10px] font-bold flex items-center gap-0.5"><CheckCircle size={10}/> Linked</span>}
                               </div>
                               {!student.youtubeUrl ? (
                                 <input 
                                   type="text" 
                                   defaultValue={student.youtubeUrl || ''} 
                                   onBlur={(e) => {
                                     if (e.target.value !== (student.youtubeUrl || '')) {
                                       updateDoc(getPublicDoc('students', student.id), { youtubeUrl: e.target.value }).catch(console.error);
                                     }
                                   }}
                                   className="w-full border border-gray-300 rounded p-1.5 text-xs outline-none focus:ring-1 focus:ring-indigo-500 bg-white shadow-sm" 
                                   placeholder="Paste YouTube link..."
                                 ...
                                 <div className="flex gap-1.5">
                                    <a href={youtubeUtils.getStandardUrl(student.youtubeUrl)} target="_blank" rel="noopener noreferrer" className="flex-1 bg-white border border-gray-300 text-xs font-bold text-gray-700 py-1.5 rounded flex items-center justify-center hover:bg-gray-50 shadow-sm transition-colors">
                                      Test Link
                                    </a>
                                    <button onClick={() => updateDoc(getPublicDoc('students', student.id), { youtubeUrl: '' }).catch(console.error)} className="bg-white border border-gray-300 text-red-500 px-2.5 py-1.5 rounded hover:bg-red-50 shadow-sm transition-colors" title="Remove Link">
                                      <Trash2 size={14}/>
                                    </button>
                                 </div>
                               )}
