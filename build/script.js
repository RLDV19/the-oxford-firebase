...
// Lucide Icons from Shim
const { Trash2, Plus, Save, UserCircle, Settings, Award, ShieldCheck, GraduationCap, FileText, Star, Users, LogOut, ArrowLeft, UserPlus, Clock, CheckCircle, Edit3, Send, Image: ImageIcon, X, Move, Search, Folder, ChevronDown, ChevronRight, Calendar, Link: LinkIcon, Phone, Mail, Tag, Lock, MessageSquare, ChevronLeft, Sparkles, UserX, Activity, TrendingUp, Archive, Cloud, ClipboardList } = window.LucideReactShim || {};

// --- FIREBASE IMPORTS ---
// Using window globals provided by index.html script type="module"
const { initializeApp } = window.firebase;
const { getAuth, signInWithCustomToken, signInAnonymously, onAuthStateChanged } = window.FirebaseAuth;
const { getFirestore, collection, doc, setDoc, getDoc, updateDoc, deleteDoc, onSnapshot, writeBatch, query } = window.FirebaseFirestore;
...
const getPublicDoc = (collectionName, documentId) => {
    return doc(firestoreDb, 'artifacts', appId, 'public', 'data', collectionName, documentId);
};
// --- CONSTANTS & HELPERS ---
...
const container = document.getElementById('report-container');
        ...
        const noPdfElements = container.querySelectorAll('.no-pdf');
        noPdfElements.forEach(el => el.classList.add('hidden'));

        const pages = document.querySelectorAll('.report-page');
        if (pages.length === 0) {
           throw new Error("No report pages found in DOM");
        }
...
function AttendanceSummaryReport({ student, onClose }) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
        ...
        if (!container) {
          throw new Error("Attendance report container not found");
        }
...
function App() {
  ...
  useEffect(() => {
      ...
      try {
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
          ...
          try {
            ...
            console.warn("Anonymous auth failed, using guest fallback", e);
            // Set dummy user to trigger data sync
            setFirebaseUser({ uid: 'guest', isAnonymous: true });
          }
        }
      } catch (error) {
        console.error('Auth error:', error);
        // Set dummy user to trigger data sync even on fatal auth error
        setFirebaseUser({ uid: 'guest', isAnonymous: true });
      }
    };
    initAuth();
    ...
    // Safety timeout: If DB doesn't load in 5 seconds, force it.
    const timer = setTimeout(() => {
      setIsDbLoaded(prev => {
        if (!prev) {
          console.warn("Forcing app load due to timeout");
          // Ensure we have a user state so the next effect runs
          setFirebaseUser(prevUser => prevUser || { uid: 'guest', isAnonymous: true });
          return true;
        }
        return prev;
      });
  ...
  useEffect(() => {
    // Need either a user OR a forced load state to proceed
    if (!firebaseUser && !isDbLoaded) return;
    ...
    const seedDbIfNeeded = async () => {
      ...
      isSeeding = true;
      try {
        // Only try to read if we have a real user, otherwise skip to seeding default
        const shouldUseFirestore = firebaseUser && firebaseUser.uid !== 'guest';
        ...
        if (shouldUseFirestore) {
            try {
                ...
                if (snap.exists()) shouldSeed = false;
            } catch (err) {
                console.warn("Firestore read failed, falling back to local defaults", err);
            }
        } else {
            console.log("Guest mode: Using local default data");
        }
        ...
        if (shouldSeed) {
          ...
          // In guest mode (no auth), we can't write to Firestore usually.
          // So we just set local state.
          // But if we are admin, we write.
    ...
    if (firebaseUser && firebaseUser.uid !== 'guest') {
        ...
        // If guest, ensure we have initial DB state at least
        setDb(getInitialDb());
    }
  }, [firebaseUser, isDbLoaded]);
  ...
  const isGuest = firebaseUser && firebaseUser.isAnonymous && firebaseUser.uid === 'guest';

  // --- SAFE FIRESTORE WRAPPERS ---
const getGuestPath = (ref) => {
      if (!ref || !ref.path) return null;
      const parts = ref.path.split('/');
      // The collection is at index -2, id at -1
      if (parts.length >= 2) {
          return { col: parts[parts.length - 2], id: parts[parts.length - 1] };
      }
      return null;
  };

  // Helper for deep merging objects in Guest Mode
  const deepMerge = (target, source) => {
    if (typeof target !== 'object' || target === null || typeof source !== 'object' || source === null) {
      return source;
    }
    
    // If array, replace (Firestore 'merge' typically replaces arrays unless arrayUnion used)
  ...
  const safeUpdateDoc = async (ref, data) => {
      try {
        ...
        console.warn("safeUpdateDoc failed, falling back to guest logic", e);
        const pathInfo = getGuestPath(ref);
        if (!pathInfo) return Promise.resolve();
        const { col, id } = pathInfo;
        console.log(`[Guest Mode] Simulating updateDoc on ${col}/${id}`, data); 

        setDb(prev => {
          // Handle special settings document
          if (col === 'settings' && id === 'global') {
             return { ...prev, ...data };
          }

          // Handle collections - Ensure collection exists
          const list = prev[col] || [];
          
          // If list was empty/missing, we can't update a specific doc in it, but we return the valid structure
          if (list.length === 0) return { ...prev, [col]: [] };

          return {
            ...prev,
            [col]: list.map(item => item.id === id ? { ...item, ...data } : item)
          };
        });
    ...
    const safeSetDoc = async (ref, data, options) => {
      try {
        await setDoc(ref, data, options);
      } catch (e) {
        console.warn("safeSetDoc failed, falling back to guest logic", e);
        const pathInfo = getGuestPath(ref);
        if (!pathInfo) return Promise.resolve();
        const { col, id } = pathInfo;
        console.log(`[Guest Mode] Simulating setDoc on ${col}/${id}`, data);    

        setDb(prev => {
          if (col === 'settings' && id === 'global') {
             if (options && options.merge) return deepMerge(prev, data);
             return { ...prev, ...data };
          }
          ...
          if (index > -1) {
            if (options && options.merge) {
               ...
               // Use deepMerge for existing item
               updatedList[index] = deepMerge(updatedList[index], data);
               return { ...prev, [col]: updatedList };
            }
            // Overwrite
            const updatedList = [...list];
            updatedList[index] = { id, ...data };
            return {
              ...prev,
              [col]: updatedList
            };
    ...
    const safeDeleteDoc = async (ref) => {
      try {
        await deleteDoc(ref);
      } catch (e) {
        console.warn("safeDeleteDoc failed, falling back to guest logic", e);
        const pathInfo = getGuestPath(ref);
        if (!pathInfo) return Promise.resolve();
        const { col, id } = pathInfo;
        console.log(`[Guest Mode] Simulating deleteDoc on ${col}/${id}`);
        ...
        setDb(prev => {
             ...
             return {
                 ...prev,
                 [col]: list.filter(item => item.id !== id)
             };
        });
        return Promise.resolve();
      }
  ...
  const handleUpdateStudentInfo = (studentId, newData) => {
    safeUpdateDoc(getPublicDoc('students', studentId), newData).catch(console.error);
  };
  ...
  const handleUpdateUserSettings = (userId, { permissions, calendarUrl }) => {
    ...
    safeUpdateDoc(getPublicDoc('users', userId), { permissions, calendarUrl: finalUrl }).then(() => {
  ...
  const handleSaveAdminCalendarUrl = () => {
    ...
    safeSetDoc(getPublicDoc('settings', 'global'), { adminCalendarUrl: finalUrl }, { merge: true }).then(() => {
  ...
  const updateReportData = (studentId, reportId, newData) => {
    ...
    safeUpdateDoc(getPublicDoc('students', studentId), { reports: updatedReports }).catch(console.error);
  };
  ...
  const deleteReport = (studentId, reportId) => {
    const student = db.students.find(s => s.id === studentId);
    if (!student) return;
    safeUpdateDoc(getPublicDoc('students', studentId), { reports: student.reports.filter(r => r.id !== reportId) }).catch(console.error);
  };
  ...
  const createNewReport = (studentId, courseType = 'PHONICS') => {
    ...
    safeUpdateDoc(getPublicDoc('students', studentId), { reports: [...(student.reports || []), newReport] }).catch(console.error);
  };

  const assignStudentToTeacher = (studentId, teacherId) => {
    safeUpdateDoc(getPublicDoc('students', studentId), { teacherId }).catch(console.error);
  };
  ...
  const handleRequestCertificate = (studentId) => {
    showModal('CONFIRM', {
      ...
      onConfirm: () => {
        safeUpdateDoc(getPublicDoc('students', studentId), { 
  ...
  const handleApproveCertificate = (studentId) => {
    ...
    // Check against local state if we are in guest mode, otherwise use cloud state (db IS local state effectively in guest mode)
  ...
  const handleCertTemplateUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        safeSetDoc(getPublicDoc('settings', 'global'), { certificateTemplate: reader.result }, { merge: true }).catch(console.error);
      };
      reader.readAsDataURL(file);
    } else {
        safeSetDoc(getPublicDoc('settings', 'global'), { certificateTemplate: null }, { merge: true }).catch(console.error);
    }
  };

  const handleUpdateCertSettings = (key, val) => {
    safeSetDoc(getPublicDoc('settings', 'global'), { certificateSettings: { ...db.certificateSettings, [key]: val } }, { merge: true }).catch(console.error);
  };
  ...
  const openAddStudentModal = () => {
    showModal('ADD_STUDENT', {
      onSubmit: (newStudentName) => {
        ...
        safeSetDoc(getPublicDoc('students', newStudent.id), newStudent).catch(console.error);
      }
    });
  };
  ...
  const openAddUserModal = () => {
    showModal('ADD_USER', {
      onSubmit: ({ name, username, password, role }) => {
        ...
        safeSetDoc(getPublicDoc('users', newUser.id), newUser).catch(console.error);
      }
    });
  };
    ...
    showModal('CONFIRM', {
      ...
      onConfirm: async () => {
        try {
          ...
          console.warn("Delete user failed, falling back to local simulation", e);
          setDb(prev => {
              const updatedUsers = prev.users.filter(u => u.id !== userId);
              const updatedStudents = prev.students.map(s => s.teacherId === userId ? { ...s, teacherId: null } : s);
              return { ...prev, users: updatedUsers, students: updatedStudents };
          });
          console.log('[Guest Mode] Simulated batch delete');
        }
      }
    });
  };
  ...
  const handleAdminPhotoUpload = (type, id, e) => {
    ...
    reader.onloadend = () => {
      if (type === 'user') {
        safeUpdateDoc(getPublicDoc('users', id), { profileImage: reader.result }).catch(console.error);
      } else if (type === 'student') {
        const student = db.students.find(s => s.id === id);
        if(!student) return;
        const updatedReports = student.reports.map(r => ({ ...r, data: { ...r.data, profileImage: reader.result } }));
        safeUpdateDoc(getPublicDoc('students', id), { profileImage: reader.result, reports: updatedReports }).catch(console.error);
      }
    };
    reader.readAsDataURL(file);
  };
    ...
    if (file && currentStudentId && currentReportId) {
      ...
      reader.onloadend = () => {
        if (field === 'profileImage') {
          ...
          if(!student) return;
          const updatedReports = student.reports.map(r => ({ ...r, data: { ...r.data, profileImage: reader.result } }));
          safeUpdateDoc(getPublicDoc('students', currentStudentId), { profileImage: reader.result, reports: updatedReports }).catch(console.error);
  ...
  const handleRemoveProfileImage = () => {
    ...
    safeUpdateDoc(getPublicDoc('students', currentStudentId), { profileImage: null, reports: updatedReports }).catch(console.error);
                  ...
                  <div className="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col h-[750px]">
                    <div className="p-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center shrink-0">
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <button onClick={() => showModal('EVENT_MODAL', { date: new Date().toISOString().split('T')[0], currentUser, teachers: db.users.filter(u => u.role === 'teacher'), onSave: (evt) => safeSetDoc(getPublicDoc('events', evt.id), evt, { merge: true }) })} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm flex items-center gap-2 transition-colors"><Plus size={18}/> New Event</button>
                      </div>
                    </div>
                    <div className="flex-1 bg-gray-50 overflow-hidden flex flex-col p-4 relative">
                      {db.adminCalendarUrl ? (
                        <div className="absolute inset-0 z-50 bg-white">
                          <div className="bg-indigo-50 border-b border-indigo-200 p-2 flex justify-between text-xs px-4"><span className="text-indigo-800 font-medium">Viewing External Calendar.</span><button onClick={() => safeSetDoc(getPublicDoc('settings', 'global'), { adminCalendarUrl: '' }, { merge: true })} className="text-indigo-600 font-bold hover:underline">Unlink</button></div>
              ...
              <div className="w-full max-w-5xl">
                ...
                {teacherTab === 'students' ? (
                  ...
                  <div className="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden flex flex-col h-[750px]">
                    <div className="p-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center shrink-0"><div><h3 className="text-xl font-bold text-[#213568] flex items-center gap-2 mb-1"><Calendar size={22} className="text-indigo-600" /> My Schedule</h3></div>{!currentUser.calendarUrl && <button onClick={() => showModal('EVENT_MODAL', { date: new Date().toISOString().split('T')[0], currentUser, teachers: db.users.filter(u => u.role === 'teacher'), onSave: (e) => safeSetDoc(getPublicDoc('events', e.id), e) })} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm flex items-center gap-2 transition-colors"><Plus size={18}/> Add Class</button>}</div>
